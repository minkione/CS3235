    ...
    public Request authenticate(Proxy proxy, Response response) throws IOException {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if ("Basic".equalsIgnoreCase(challenge.getScheme())) {
                ...
                InetAddress connectToInetAddress = getConnectToInetAddress(proxy, url);
                int port = url.getPort();
                String protocol = url.getProtocol();
                String realm = challenge.getRealm();
                String scheme = challenge.getScheme();
                PasswordAuthentication requestPasswordAuthentication = java.net.Authenticator.requestPasswordAuthentication(host, connectToInetAddress, port, protocol, realm, scheme, url, RequestorType.SERVER);
