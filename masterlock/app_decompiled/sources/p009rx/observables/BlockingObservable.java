...
import p009rx.internal.operators.BlockingOperatorLatest;
import p009rx.internal.operators.BlockingOperatorMostRecent;
import p009rx.internal.operators.BlockingOperatorNext;
import p009rx.internal.operators.BlockingOperatorToFuture;
import p009rx.internal.operators.BlockingOperatorToIterator;
import p009rx.internal.operators.NotificationLite;
import p009rx.internal.util.BlockingUtils;
...
public final class BlockingObservable<T> {
    ...
    /* renamed from: o */
    private final Observable<? extends T> f272o;

    private BlockingObservable(Observable<? extends T> observable) {
        this.f272o = observable;
    }
    ...
    public void forEach(final Action1<? super T> action1) {
        ...
        BlockingUtils.awaitForComplete(countDownLatch, this.f272o.subscribe((Subscriber<? super T>) new Subscriber<T>() {
    ...
    public Iterator<T> getIterator() {
        return BlockingOperatorToIterator.toIterator(this.f272o);
    }

    public T first() {
        return blockForSingle(this.f272o.first());
    }

    public T first(Func1<? super T, Boolean> func1) {
        return blockForSingle(this.f272o.first(func1));
    }

    public T firstOrDefault(T t) {
        return blockForSingle(this.f272o.map(UtilityFunctions.identity()).firstOrDefault(t));
    }

    public T firstOrDefault(T t, Func1<? super T, Boolean> func1) {
        return blockForSingle(this.f272o.filter(func1).map(UtilityFunctions.identity()).firstOrDefault(t));
    }

    public T last() {
        return blockForSingle(this.f272o.last());
    }

    public T last(Func1<? super T, Boolean> func1) {
        return blockForSingle(this.f272o.last(func1));
    }

    public T lastOrDefault(T t) {
        return blockForSingle(this.f272o.map(UtilityFunctions.identity()).lastOrDefault(t));
    }

    public T lastOrDefault(T t, Func1<? super T, Boolean> func1) {
        return blockForSingle(this.f272o.filter(func1).map(UtilityFunctions.identity()).lastOrDefault(t));
    }

    public Iterable<T> mostRecent(T t) {
        return BlockingOperatorMostRecent.mostRecent(this.f272o, t);
    }

    public Iterable<T> next() {
        return BlockingOperatorNext.next(this.f272o);
    }

    public Iterable<T> latest() {
        return BlockingOperatorLatest.latest(this.f272o);
    }

    public T single() {
        return blockForSingle(this.f272o.single());
    }

    public T single(Func1<? super T, Boolean> func1) {
        return blockForSingle(this.f272o.single(func1));
    }

    public T singleOrDefault(T t) {
        return blockForSingle(this.f272o.map(UtilityFunctions.identity()).singleOrDefault(t));
    }

    public T singleOrDefault(T t, Func1<? super T, Boolean> func1) {
        return blockForSingle(this.f272o.filter(func1).map(UtilityFunctions.identity()).singleOrDefault(t));
    }

    public Future<T> toFuture() {
        return BlockingOperatorToFuture.toFuture(this.f272o);
    }
    ...
    private T blockForSingle(Observable<? extends T> observable) {
        ...
        final CountDownLatch countDownLatch = new CountDownLatch(1);
        BlockingUtils.awaitForComplete(countDownLatch, observable.subscribe((Subscriber<? super T>) new Subscriber<T>() {
            public void onCompleted() {
                countDownLatch.countDown();
            }
    ...
    public void subscribe(Observer<? super T> observer) {
        ...
        Subscription subscribe = this.f272o.subscribe((Subscriber<? super T>) new Subscriber<T>() {
            public void onNext(T t) {
    ...
    public void subscribe(Subscriber<? super T> subscriber) {
        ...
        this.f272o.subscribe((Subscriber<? super T>) r3);
        while (true) {
            try {
