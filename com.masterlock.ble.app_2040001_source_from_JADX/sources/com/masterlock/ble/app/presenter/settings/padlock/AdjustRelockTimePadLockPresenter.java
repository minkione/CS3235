...
import com.masterlock.ble.app.view.settings.padlock.AdjustRelockTimePadLockView;
import com.masterlock.core.Lock;
import com.masterlock.core.LockConfigAction;
import com.square.flow.appflow.AppFlow;

public class AdjustRelockTimePadLockPresenter extends AuthenticatedPresenter<Lock, AdjustRelockTimePadLockView> {
    public static final int MINIMUM_RELOCK_TIME = 4;
    private SeekBar mRelockSeekBar;
    /* access modifiers changed from: private */
    public TextView mRelockTimeTextView;

    public AdjustRelockTimePadLockPresenter(AdjustRelockTimePadLockView adjustRelockTimePadLockView) {
        super(adjustRelockTimePadLockView);
        this.model = ((AdjustRelockTimePadLock) AppFlow.getScreen(adjustRelockTimePadLockView.getContext())).mLock;
    }

    public void start() {
        super.start();
        this.mEventBus.post(new Builder(((AdjustRelockTimePadLockView) this.view).getResources()).build());
        setUpHeaderView();
    }
    ...
    public void updateRelockTime(int i) {
        ...
        AppFlow.get(((AdjustRelockTimePadLockView) this.view).getContext()).goTo(new ApplyChanges((Lock) this.model, C1075R.string.title_adjust_relock_time, LockConfigAction.RELOCK_TIME));
    }
    ...
    private OnSeekBarChangeListener getSeekBarListener() {
        return new OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int i, boolean z) {
                AdjustRelockTimePadLockPresenter.this.mRelockTimeTextView.setText(String.valueOf(i + 4));
            }
        };
    }

    private void showLockerModeDialog() {
        SimpleDialog simpleDialog = new SimpleDialog(((AdjustRelockTimePadLockView) this.view).getContext(), null);
        Dialog dialog = new Dialog(((AdjustRelockTimePadLockView) this.view).getContext());
