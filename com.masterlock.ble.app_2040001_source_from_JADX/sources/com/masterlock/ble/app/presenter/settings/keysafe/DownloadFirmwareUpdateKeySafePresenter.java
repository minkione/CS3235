...
import com.masterlock.ble.app.view.settings.keysafe.DownloadFirmwareUpdateKeySafeView;
...
public class DownloadFirmwareUpdateKeySafePresenter extends AuthenticatedPresenter<Lock, DownloadFirmwareUpdateKeySafeView> {
    ...
    public DownloadFirmwareUpdateKeySafePresenter(Lock lock, DownloadFirmwareUpdateKeySafeView downloadFirmwareUpdateKeySafeView) {
        super(lock, downloadFirmwareUpdateKeySafeView);
    }
    ...
    public void downloadFirmwareUpdate() {
        ...
        this.mUpdateSubscription = this.mLockService.getFirmwareUpdate((Lock) this.model).subscribeOn(this.mScheduler.background()).observeOn(this.mScheduler.main()).subscribe((Subscriber<? super T>) new Subscriber<FirmwareUpdate>() {
            ...
            public void onCompleted() {
                DownloadFirmwareUpdateKeySafePresenter.this.mEventBus.post(new ToggleProgressBarEvent(false));
                ((DownloadFirmwareUpdateKeySafeView) DownloadFirmwareUpdateKeySafePresenter.this.view).updateViewSuccess();
                ((DownloadFirmwareUpdateKeySafeView) DownloadFirmwareUpdateKeySafePresenter.this.view).stopProgress();
                FileUtil.getInstance().saveFirmwareUpdateToFile(DownloadFirmwareUpdateKeySafePresenter.this.mFirmwareUpdate, ((Lock) DownloadFirmwareUpdateKeySafePresenter.this.model).getKmsId());
            }

            public void onError(Throwable th) {
                DownloadFirmwareUpdateKeySafePresenter.this.mEventBus.post(new ToggleProgressBarEvent(false));
                ((DownloadFirmwareUpdateKeySafeView) DownloadFirmwareUpdateKeySafePresenter.this.view).updateViewError();
                ((DownloadFirmwareUpdateKeySafeView) DownloadFirmwareUpdateKeySafePresenter.this.view).stopProgress();
            }

            public void onNext(FirmwareUpdate firmwareUpdate) {
                Log.v("Firmware", firmwareUpdate.toString());
                DownloadFirmwareUpdateKeySafePresenter.this.mFirmwareUpdate = firmwareUpdate;
    ...
    public void displayLockermodeDialog() {
        LockerModeDialogPadLock lockerModeDialogPadLock = new LockerModeDialogPadLock(((DownloadFirmwareUpdateKeySafeView) this.view).getContext(), null);
        ...
        lockerModeDialogPadLock.setPositiveButtonClickListener(new OnClickListener() {
            public final void onClick(View view) {
                DownloadFirmwareUpdateKeySafePresenter.this.mDialog.dismiss();
            }
        });
        this.mDialog = new Dialog(((DownloadFirmwareUpdateKeySafeView) this.view).getContext());
    ...
    public void sendCommand(Map<String, String> map) {
        ...
        AppFlow.get(((DownloadFirmwareUpdateKeySafeView) this.view).getContext()).goTo(new ApplyChanges((Lock) this.model, C1075R.string.title_firmware_update, LockConfigAction.UPDATE_FIRMWARE));
    }
}
