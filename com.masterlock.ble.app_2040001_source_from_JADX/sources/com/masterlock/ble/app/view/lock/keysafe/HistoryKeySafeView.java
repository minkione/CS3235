...
public class HistoryKeySafeView extends LinearLayout implements IAuthenticatedView {
    ...
    private HistoryKeySafePresenter mHistoryKeySafePresenter;
    ...
    public HistoryKeySafeView(Context context) {
        super(context);
    }

    public HistoryKeySafeView(Context context, AttributeSet attributeSet) {
        super(context, attributeSet);
    }

    public HistoryKeySafeView(Context context, AttributeSet attributeSet, int i) {
    ...
    public void onFinishInflate() {
        ...
        if (!isInEditMode()) {
            ...
            HistoryKeySafe historyKeySafe = (HistoryKeySafe) AppFlow.getScreen(getContext());
            this.mHistoryKeySafePresenter = new HistoryKeySafePresenter(historyKeySafe.mLock, this);
            this.mAdapter = new HistoryAdapter(getContext(), this.mHistoryKeySafePresenter.getModel());
            this.mListView.setAdapter(this.mAdapter);
            this.mListView.setEmptyView(this.mEmptySwipeRefreshLayout);
            this.mLockName.setText(historyKeySafe.mLock.getName());
            this.deviceId.setText(historyKeySafe.mLock.getKmsDeviceKey().getDeviceId());
            setupSwipeRefreshLayouts(this.mSwipeRefreshLayout, this.mEmptySwipeRefreshLayout);
            MasterLockApp.get().inject(this.mHistoryKeySafePresenter);
        }
    }
    ...
    public void onAttachedToWindow() {
        ...
        if (!isInEditMode()) {
            this.mHistoryKeySafePresenter.start();
        }
    }
    ...
    public void onDetachedFromWindow() {
        ...
        this.mHistoryKeySafePresenter.finish();
    }
        ...
        for (SwipeRefreshLayout swipeRefreshLayout : swipeRefreshLayoutArr) {
            swipeRefreshLayout.setOnRefreshListener(new OnRefreshListener() {
                public final void onRefresh() {
                    HistoryKeySafeView.this.mHistoryKeySafePresenter.refresh();
                }
            });
