...
final class BasicWeekyearDateTimeField extends ImpreciseDateTimeField {
    ...
    public long getDifferenceAsLong(long j, long j2) {
        ...
        if (remainder2 >= WEEK_53 && this.iChronology.getWeeksInYear(i) <= 52) {
            remainder2 -= FirmwareUpdateService.NOTIFY_INTERVAL;
        }
        int i3 = i - i2;
    ...
    public long set(long j, int i) {
        ...
        int i3 = get(year);
        if (i3 < i) {
            year += FirmwareUpdateService.NOTIFY_INTERVAL;
        } else if (i3 > i) {
            year -= FirmwareUpdateService.NOTIFY_INTERVAL;
        }
        return this.iChronology.dayOfWeek().set(year + (((long) (weeksInYear - this.iChronology.getWeekOfWeekyear(year))) * FirmwareUpdateService.NOTIFY_INTERVAL), dayOfWeek);
    }
    ...
    public long roundFloor(long j) {
        ...
        return weekOfWeekyear > 1 ? roundFloor - (((long) (weekOfWeekyear - 1)) * FirmwareUpdateService.NOTIFY_INTERVAL) : roundFloor;
    }
