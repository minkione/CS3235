...
class AdvertisingInfoProvider {
    ...
    public AdvertisingInfo getAdvertisingInfo() {
        ...
        if (isInfoValid(infoFromPreferences)) {
            Fabric.getLogger().mo21793d(Fabric.TAG, "Using AdvertisingInfo from Preference Store");
        ...
        new Thread(new BackgroundPriorityRunnable() {
            public void onRun() {
                ...
                if (!advertisingInfo.equals(access$000)) {
                    Fabric.getLogger().mo21793d(Fabric.TAG, "Asychronously getting Advertising Info and storing it to preferences");
    ...
    public AdvertisingInfo getAdvertisingInfoFromStrategies() {
        ...
        if (!isInfoValid(advertisingInfo)) {
            ...
            if (!isInfoValid(advertisingInfo)) {
                Fabric.getLogger().mo21793d(Fabric.TAG, "AdvertisingInfo not present");
            } else {
                Fabric.getLogger().mo21793d(Fabric.TAG, "Using AdvertisingInfo from Service Provider");
            }
        } else {
            Fabric.getLogger().mo21793d(Fabric.TAG, "Using AdvertisingInfo from Reflection Provider");
        }
